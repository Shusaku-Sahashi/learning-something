...
何も見ずに自分の手で実装してみましょう。**Part1 を先に完成させてから**
取り組んでください(自分で書いたジェネレータを、自分で書いた `parseJSON`
でテストする、という腕試しだからです。検証用テストは `Exercise.Part1.Parser`
に固定で結び付けられているため、Part2 だけを終えた状態では動きません)。
